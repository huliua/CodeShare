<script setup>
    import { ArrowDown, ArrowUp, Refresh, Search, Star, StarFilled } from '@element-plus/icons-vue';
    import { downloadCode, favourCode, genCode, getCodesList } from '@/api/codeShare';
     ...
     * 获取数据列表
     */
    const getDataList = function (resetPage = false) {
        if (resetPage) {
            pageNum.value = 1;
        }
        loading.value = true;
...
<template>
    <div class="list-page">
        ...
        <div class="page-header">
            <h2 class="page-title">代码库</h2>
            <p class="page-desc">浏览公开的代码片段和模板</p>
        </div>
        ...
        <div class="search-panel">
            ...
            <div class="search-actions">
                <el-button type="primary" :icon="Search" @click="getDataList(true)">搜索</el-button>
        ...
        <div v-show="!loading && dataList.length > 0" class="card-grid">
            ...
            >
                ...
                <div v-show="getIsHover(item)" class="card-actions" @click.stop>
                    ...
                    <button class="action-btn action-view" @click="goDetail(item)">查看</button>
                </div>
            </div>
        </div>
        ...
        <div v-if="!loading && dataList.length === 0" class="empty-state">
            ...
            <h3 class="empty-title">暂无公开代码</h3>
            <p class="empty-desc">快来发现更多精彩代码片段和模板</p>
            <el-button type="primary" :icon="Refresh" @click="getDataList">刷新列表</el-button>
        </div>
...
<style scoped>
    ...
    .card-cover {
        position: relative;
        height: 140px;
        background: var(--cs-card-cover-bg);
        display: flex;
        align-items: center;
        justify-content: center;
        overflow: hidden;
    ...
    .cover-icon {
        font-family: var(--cs-font-heading);
        font-size: 36px;
        color: var(--cs-card-cover-icon-color);
        opacity: 0.5;
    }
    ...
    .card-actions {
        ...
        gap: 4px;
        padding: 8px 12px;
        background: var(--cs-card-actions-bg);
